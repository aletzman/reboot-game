...
export function GameButton({
    ...
    const base = "relative inline-flex items-center justify-center gap-2 font-mono uppercase tracking-[.1em] transition-all duration-200 cursor-pointer select-none group";

    const variants: Record<string, string> = {
        command: `
            text-[10px] px-3 py-[6px]
    ...
    const accentClasses = accentColor
        ? (active
            ? "border-[var(--accent-base)]! text-[var(--accent-base)]! before:bg-[var(--accent-base)]!"
            : "border-[var(--accent-border)]! hover:border-[var(--accent-border-hover)]! hover:text-[var(--accent-base)]!"
        )
        : "";

    const disabledStyles = disabled
        ? "opacity-30 cursor-not-allowed! grayscale hover:bg-(--bg-elevated)! hover:border-(--bg-hover)! hover:shadow-none! hover:text-(--text-ghost)! active:scale-100!"
        : "";

    const dynamicStyle = accentColor ? {
        '--accent-base': accentColor,
        '--accent-border': `color-mix(in srgb, ${accentColor} 15%, transparent)`,
    ...
    return (
        ...
        >
            ...
            {variant === 'command' && (
                <div className={`absolute top-1.5 right-1.5 w-1 h-1 rounded-full transition-all duration-300 ${
                    active 
