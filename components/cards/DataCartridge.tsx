                ...
                <div className={`w-full h-full relative transition-transform duration-600 ease-in-out`}
                    ...
                    <div className="absolute inset-0 flex flex-col overflow-hidden font-mono"
                        ...
                        <div className="flex-1 m-2 bg-black border border-white/5 shadow-inner relative overflow-hidden p-3 flex flex-col justify-center items-center">
                            ...
                            {/* ID Mark */}
                            <div className={`absolute left-2 text-[6px] text-white/40 tracking-widest font-mono ${detailed ? 'top-10' : 'top-2'}`} style={{ transform: 'translateZ(0.1px)' }}>
                            ...
                            <div className="flex flex-col gap-1 items-center justify-center relative z-10 w-full px-2 mt-4 flex-1">
                                <h3 className={`font-normal uppercase  text-white leading-none text-center ${detailed ? 'text-2xl mb-1' : 'text-[14px]'}`} style={{ textShadow: '0 0 10px rgba(255,255,255,0.1)' }}>
                                    Velocidad de tecleo
                                </h3>
                                <div className="h-px w-8 bg-current opacity-30 my-1" style={{ color: rarity.color }} />
                                <div className={`text-(--text-muted) uppercase font-normal tracking-wider text-center ${detailed ? 'text-xs' : 'text-[10px]'}`}
                                    style={{ transform: 'translate3d(0, 0, 0.5px)', textRendering: 'geometricPrecision' }}>
                                    {card.concept}
                                </div>
                            </div>
                            ...
                            {/* Rarity dots */}
                            <div className={`absolute flex gap-1 ${detailed ? 'bottom-5' : 'bottom-1'}`}>
                                {[...Array(4)].map((_, i) => (
                                    <div key={i}
                    ...
                    <div className="absolute inset-0 flex flex-col p-1.5"
                        style={{
                            backfaceVisibility: 'hidden',
                            WebkitBackfaceVisibility: 'hidden',
                            transform: 'rotateY(180deg) translate3d(0, 0, 6px)',
                        ...
                        <div className="h-8 w-full bg-[#050608] flex items-center justify-between px-3 mt-1 mb-2 border border-white/5 rounded-xs relative overflow-hidden">
                            ...
                            <span className="text-[10px] text-(--green-light) tracking-[.25em] font-bold z-10">CORE_LOG / DATABANK</span>
                        ...
                        <div className="flex-1 flex flex-col gap-3 overflow-hidden px-2 py-1 font-mono relative">
                            <div className={`font-normal uppercase tracking-wider flex items-center gap-2 shrink-0 ${detailed ? 'text-[14px]' : 'text-[12px]'}`}
                                style={{ color: rarity.color, transform: 'translate3d(0, 0, 0.5px)', textRendering: 'geometricPrecision' }}>
                                <span className="shrink-0">[{card.concept}]</span>
                                <span className="h-px flex-1 bg-current opacity-20"></span>
                            </div>

                            <div className={`${detailed ? 'text-[12px]' : 'text-[10px]'} text-(--text-muted) leading-relaxed flex-1 overflow-y-auto custom-scrollbar pr-1 relative z-10 flex flex-col gap-3 pb-1`}
