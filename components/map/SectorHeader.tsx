...
export function SectorHeader({ actId, actName, children }: SectorHeaderProps) {
  return (
    <div className="sticky top-0 pt-2 z-50 flex flex-col gap-2.5 mb-4 w-full animate-in fade-in slide-in-from-top-1 duration-500 ease-out bg-(--bg-void)">
      ...
      <div className="flex items-center justify-between w-full pb-2 border-b border-[#1a2636]/30">
        <NavButton href="/game" icon={ChevronLeftIcon} className="scale-90 origin-left">
          MAPA_SECTORES
        </NavButton>

        <div className="flex items-center gap-4 font-mono text-[9px] text-(--text-muted)/40">
          <div className="flex items-center gap-1.5">
            <span className="w-1 h-1 rounded-full bg-(--green-base) animate-pulse shadow-[0_0_4px_var(--green-base)]" />
      ...
      <div className="relative">
        <div className="absolute -inset-0.5 bg-linear-to-r from-(--green-base)/20 via-transparent to-transparent opacity-20 transition-opacity" />
        ...
        <header className="relative w-full bg-[#080c11]/80 border border-[#1a2636]/60 rounded-xs overflow-hidden flex flex-col md:flex-row items-stretch">
          ...
          {/* Accent Line */}
          <div className="absolute top-0 left-0 w-full h-px bg-linear-to-r from-(--green-base)/60 via-(--green-base)/10 to-transparent" />
          <div className="absolute top-0 left-0 w-1 h-full bg-(--green-base) shadow-[0_0_15px_var(--green-base)]" />

          {/* Tactical Corner Accents */}
          <div className="absolute top-0 left-1 w-2 h-2 border-t border-l border-(--green-muted)/30" />
          <div className="absolute bottom-0 left-1 w-2 h-2 border-b border-l border-(--green-muted)/30" />

          {/* Title Area - SERVER RENDERED */}
          <div className="flex-1 p-5 md:p-6 flex flex-col justify-center gap-0.5 min-w-0 relative">
            <div className="flex items-center gap-2 mb-1">
              <div className="flex items-center gap-1.5 px-2 py-1 bg-(--green-darkest) border border-(--green-base)/20 rounded-xs">
                <div className="w-1 h-1 rounded-full bg-(--green-light) animate-pulse shadow-[0_0_5px_var(--green-light)]" />
                <span className="text-[10px] text-(--green-light) font-black tracking-[0.2em] uppercase leading-none">
                  SECTOR_{actId.padStart(2, '0')}
                </span>
              </div>
              <span className="text-[8px] text-(--text-ghost) font-mono uppercase tracking-[0.2em]">PATH_READY</span>
            </div>
            ...
            <div className="flex items-center gap-2 mt-1 opacity-40">
              ...
              <span className="text-[7px] font-mono text-(--text-muted) tracking-[0.3em] uppercase">SYSTEM_INITIALIZED</span>
            </div>
          </div>
          ...
          <div className="absolute bottom-0 left-0 w-full h-px bg-linear-to-r from-(--green-base)/30 via-transparent to-transparent" />
        </header>
        ...
        <div className="mt-2 flex justify-between items-center px-1">
          ...
          <span className="text-[7px] font-mono text-(--text-muted)/30 tracking-[0.4em] uppercase">
            REBOOT_OS // SECTOR_MAP_INTERFACE_V1.0
          </span>
        </div>
      </div>
    </div>
