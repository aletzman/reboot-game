...
export function TheoryText({ text }: TheoryTextProps) {
    ...
    const termPattern = new RegExp(`\\b(${sortedTerms.map(t => t.term).join('|')})\\b`, 'gi')
    ...
    return (
        <span>
            {parts.map((part, i) => {
                ...
                if (term) {
                    return <TheoryTerm key={i} term={term} />
                }
                return part
            })}
        </span>
...
function TheoryTerm({ term }: { term: GlossaryTerm }) {
    ...
    return (
        <span className="inline-block">
            ...
            >
                {term.term}
                <HelpCircle size={10} className="stroke-3 opacity-60" />
            </motion.span>
            ...
            {isOpen && portalNode && createPortal(
                <div className="fixed inset-0 z-1000 pointer-events-none">
                    ...
                    >
                            ...
                            <div className="relative flex flex-col gap-4">
                                ...
                                <div className="flex bg-(--bg-deep) p-1 border border-(--bg-hover) rounded-xs">
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 py-2 text-[10px] uppercase font-bold transition-all ${
                                            mode === 'technical' 
                                ...
                                <div className="min-h-[60px] flex items-center">
                                    <p className="text-(--text-primary) text-[13px] leading-relaxed italic font-medium">
                        ...
                        <div className={`absolute left-1/2 -translate-x-1/2 w-3 h-3 bg-(--bg-elevated) border-r border-b border-(--green-base)/40 rotate-45 z-10 ${
                            coords.showBelow ? 'bottom-full -mb-1.5 rotate-[-135deg] border-none bg-(--bg-elevated)' : 'top-full -mt-1.5'
                        }`} />
                        {coords.showBelow && (
